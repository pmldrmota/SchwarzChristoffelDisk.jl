...
gauss_quadrature(integrand, zm, zd, (xq, wq)) =
	zd * mapreduce(
		((xqk, wqk),) -> integrand(zm + zd * xqk) * wqk,
		+,
        zip(xq, wq),
	)

function sc_integrate(f, za, zb, kb::Nothing)
	gauss_quadrature(zv -> sc_first_derivative(f, zv),
		(zb + za) / 2,
		(zb - za) / 2,
...
function sc_integrate(f, za, zb, kb)
	...
	if isnothing(jacobi_quadrule)
		...
		zd = (zb - za) / 2
		zm = (zb + za) / 2
		gq = gauss_quadrature(zv -> sc_first_derivative(f, zv, kb),
			zm,
			zd,
			jacobi_quadrule)
		abs(zd)^(-f.β[kb]) * gq
...
function sc_segment(f, ka)
	...
	zhalf = 0
	sc_compound_gauss_jacobi(f, zhalf, f.z[kb], kb) - sc_compound_gauss_jacobi(f, zhalf, f.z[ka], ka)
end
...
function sc_test_ok(f, w)
	sum(abs2(sc_trafo(f, f.z[k]) - w[k]) for (k, wk) ∈ enumerate(w) if !isinf(wk)) < length(w)^2 * 1e-6
end
